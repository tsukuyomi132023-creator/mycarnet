...
const qrAdminToggle = document.getElementById('qrAdminToggle');
...
const qrConfigHint = document.getElementById('qrConfigHint');
let qrLastDownloadUrl = null;
let qrLastFileName = '';
let isAdminUnlocked = false;

// NOTA: esta app es 100% frontend (sin servidor), así que este "modo administrador"
// es solo una barrera de uso pensada para evitar ediciones accidentales de la API key,
// no una medida de seguridad real: cualquier código de acceso incluido aquí es visible
// en el código fuente. Para una protección real haría falta un backend con autenticación.
const ADMIN_PASSCODE = 'ciberheroes-admin';
...
function setKeyFieldsEnabled(enabled){
  qrKeyInput.disabled = !enabled;
  qrToggleVisibility.disabled = !enabled;
  qrSaveKeyBtn.disabled = !enabled;
  if(!enabled){
    // Al bloquear, siempre se vuelve a ocultar el valor.
    qrKeyInput.type = 'password';
    qrEyeIcon.innerHTML = EYE_OPEN;
  }
}

function setAdminUnlocked(unlocked){
  isAdminUnlocked = unlocked;
  setKeyFieldsEnabled(unlocked);
  qrAdminToggle.classList.toggle('unlocked', unlocked);
  qrAdminToggle.textContent = unlocked ? '🔓 Modo administrador (activo)' : '🔒 Modo administrador';
  qrConfigHint.innerHTML = unlocked
    ? 'Modo administrador activo: puedes editar, mostrar o volver a guardar la API key las veces que necesites.'
    : 'Solo un administrador puede ver, modificar o reemplazar esta API key. Actívalo con el botón de arriba. Consíguela gratis en <b>api.imgbb.com</b>.';
}

qrAdminToggle.addEventListener('click', ()=>{
  if(isAdminUnlocked){
    setAdminUnlocked(false);
    showToast('Modo administrador desactivado');
    return;
  }
  const attempt = window.prompt('Ingresa el código de administrador para editar la API key:');
  if(attempt === null) return; // cancelado
  if(attempt === ADMIN_PASSCODE){
    setAdminUnlocked(true);
    showToast('Modo administrador activado');
  }else{
    showToast('Código de administrador incorrecto', true);
  }
});

qrToggleVisibility.addEventListener('click', ()=>{
  if(qrToggleVisibility.disabled) return;
  const showing = qrKeyInput.type === 'text';
  qrKeyInput.type = showing ? 'password' : 'text';
  qrEyeIcon.innerHTML = showing ? EYE_OPEN : EYE_CLOSED;
});

// Guardar (y volver a guardar / actualizar) la API key: sin límite de veces,
// siempre disponible mientras el modo administrador esté activo.
...
// modo administrador está activo) para no depender de que recuerde presionar el botón.
...
document.getElementById('btnShowQR').addEventListener('click', async ()=>{
  ...
  if(apiKey && isAdminUnlocked) saveApiKey(apiKey);

  if(!apiKey){
    statusEl.classList.add('error');
    statusEl.textContent = isAdminUnlocked
      ? 'Pega tu API key de ImgBB arriba y presiona "Guardar" para poder generar el QR.'
      : 'Aún no hay una API key configurada. Pide a un administrador que active "Modo administrador" y la guarde.';
    ...
    if(isAdminUnlocked) qrKeyInput.focus();
    return;
  }
  ...
  try{
    ...
    disclaimer.textContent = 'El carnet se subió a ImgBB en calidad completa y el QR solo contiene ese enlace (por eso es un código simple, fácil de leer para cualquier celular). El enlace queda accesible para quien lo tenga, así que usa "Borrar esta imagen" en cuanto ya no lo necesites.';
