  ...
  return `
    ...
    <div class="carnet">
      ...
      <div class="phrase-row">
        ...
        <div class="qr-area">
          <div id="qr-${code}" class="qr-canvas-wrap"></div>
        </div>
      </div>
      <div class="carnet-body">
        ...
        <div class="carnet-info">
          ...
          <div class="carnet-row"><span class="lbl">Escuela</span><span class="val">${escapeHtml(data.seccion) || '—'}</span></div>
        </div>
      </div>
...
/* ---------------- QR de verificación embebido en el carnet ---------------- */
// Construye el enlace a la pantalla de verificación en 2 pasos (verificar.html),
// pasándole el nombre y el código del carnet como texto.
...
// Dibuja el QR de verificación dentro del área reservada en la plantilla del
// carnet (#qr-<codigo>). Se usa tanto en la vista previa como en la galería
// y en cada exportación (PNG/JPG/PDF), para que el QR quede siempre grabado
// dentro de la imagen del carnet.
// El QR se genera en alta resolución (QR_NATIVE_SIZE) y luego se reduce con
// CSS (.qr-canvas-wrap) al tamaño visible real del carnet: así se ve pequeño
// y prolijo, pero conserva la nitidez necesaria para poder escanearse bien.
const QR_NATIVE_SIZE = 2000; // tamaño nativo del QR
function renderCardQR(scopeEl, data){
  const code = data.uid || 'CS-000000';
  const qrDiv = scopeEl.querySelector(`#qr-${code}`);
  if(!qrDiv) return;
  qrDiv.innerHTML = '';
  new QRCode(qrDiv, {
    text: buildVerifyUrl(data),
    width: QR_NATIVE_SIZE,
    height: QR_NATIVE_SIZE,
...
function renderPreview(){
  ...
  preview.innerHTML = cardHTML(currentData);
  renderCardQR(preview, currentData);
}
...
function renderGallery(){
  ...
  gallery.forEach(item=>{
    const thumb = grid.querySelector(`.gallery-item[data-id="${item.id}"] .thumb-scale`);
    if(thumb) renderCardQR(thumb, item.data);
  });
